...
A execução é orquestrada pelo **`docker compose` da raiz do repositório**, que sobe este
cluster (`chaves-n1/n2/n3`) junto com o banco central e as instituições, todos na rede
`dsid`:

```bash
# na raiz do repositório
docker compose up --build      # sobe os 3 nós (RMI do n1 publicado em 1099)
docker compose down            # derruba (volumes preservam os dados)
docker compose down -v         # derruba e apaga os dados
```

- Cada nó é um container e acha os outros pelo nome do serviço (`chaves-n1/n2/n3`) — o
  compose define `RAFT_HOST_N1=chaves-n1`, etc., e cada nó anuncia o próprio nome como
  `java.rmi.server.hostname`.
- Os clientes RMI são as **instituições**, que rodam na mesma rede Docker e fazem o lookup
  pelo nome do serviço.
- Log + snapshots ficam em volumes (`chaves-n*-data`), então os dados sobrevivem ao
  `down`/`up`. O Raft precisa de maioria (2 de 3) para aceitar escritas.
